...
export const initTrackingSocket = (io: Server) => {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join-delivery', (assignmentId: string) => {
      socket.join(assignmentId);
      console.log(`Client ${socket.id} joined tracking room: ${assignmentId}`);
    });

    // Leave tracking room
    socket.on('leave-delivery', (assignmentId: string) => {
    ...
    socket.on('delivery-status-changed', (data: {
      assignmentId: string;
      donationId: string;
      status: string; // ON_THE_WAY, ARRIVED, FOOD_PICKED_UP, DELIVERING, DELIVERED
      message: string;
    }) => {
